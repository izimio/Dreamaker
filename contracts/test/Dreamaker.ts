...
const generateXRandomAccountsAndEarnings = async (x: number) => {
    ...
    for (let i = 0; i < x; i++) {
        ...
        const earning = ethers.parseEther(
...
const expectBalanceOfAccounts = async (
    dreamaker: any,
    accounts: any,
    earnings: any
) => {
    for (let i = 0; i < accounts.length; i++) {
        expect(await dreamaker.balanceOf(accounts[i].address)).to.equal(
            earnings[i]
        );
    }
};
...
describe("Dreamaker Token", function () {
    ...
    describe("Deployment - basic", function () {
        ...
        it("Should set correct owner default minting", async function () {
            ...
            expect(await dreamaker.balanceOf(owner.address)).to.equal(
    ...
    describe("Basic - transfer", function () {
        it("should transfer tokens", async function () {
            const { dreamaker, owner, otherAccount, initialSupply } =
                await deployFixture();
            await dreamaker.transfer(
                otherAccount.address,
                ethers.parseEther("100")
            );
            expect(await dreamaker.balanceOf(otherAccount.address)).to.equal(
                ethers.parseEther("100")
            );
            expect(await dreamaker.balanceOf(owner.address)).to.equal(
                ethers.parseEther(initialSupply.toString()) -
        ...
        it("should not transfer tokens if not enough balance", async function () {
            const { dreamaker, otherAccount, initialSupply } =
                await deployFixture();
            await expect(
                dreamaker.transfer(
                    otherAccount.address,
                    ethers.parseEther((initialSupply + 5).toString())
                )
    ...
    describe("Offering - dremaker", function () {
        it("Offerring - should not allow to set invalid target amount", async function () {
            ...
            await expect(dreamaker.offer([], [])).to.be.revertedWithCustomError(
                dreamaker,
                errors.InvalidInput
            );
        });
        it("Offering - basic", async function () {
            ...
            const { accounts, earnings } =
                await generateXRandomAccountsAndEarnings(5);
            await expectBalanceOfAccounts(
                dreamaker,
                accounts,
                accounts.map(() => ethers.parseEther("0"))
            );
        ...
        it("Offering - fuzzing", async function () {
            ...
            const { accounts, earnings } =
                await generateXRandomAccountsAndEarnings(numberOfAccounts);
            await expectBalanceOfAccounts(
                dreamaker,
                accounts,
                accounts.map(() => ethers.parseEther("0"))
